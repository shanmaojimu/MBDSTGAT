...
from models.MBDSTGAT import MBDSTGAT
...
def start_run(args):
    ...
    # Call method to compute and print FLOPs
    model_classifier.to(device)  # Add this line to ensure the model is on device
    # Compute FLOPs (one-time)
    model_classifier.compute_flops()  # This will print FLOPs and Params
    ...
    all_attention_scores = []  # New: Collect attn_array from all epochs
    epoch_labels = []  # New: Collect epoch labels for each record
    ...
    for epoch in range(start_epoch, args.epochs):
        record_this_epoch = True  # Modified: Record attention scores for each epoch
        node_weights, space_node_weights, avg_train_loss, train_acc_tuple, train_loss_tuple = train_one_epoch()
        # Process C3 node attention history
        if record_this_epoch and hasattr(model_classifier.ge,
                                         'attention_history') and model_classifier.ge.attention_history:
            import numpy as np
            attn_array = torch.stack(model_classifier.ge.attention_history).numpy()  # [1, 22]
            all_attention_scores.append(attn_array[0])  # Only append one row vector
            epoch_labels.append(epoch + 1)  # Start from 1
            model_classifier.ge.attention_history = []
    ...
    # save_loss_gap = '/home/fafu/lrq/EEG/KGAT-Mamba/loss_gap/HGD1/12/TPRGA-Net++(公开)'
    ...
    if all_attention_scores:
        import numpy as np
        import pandas as pd
        full_array = np.vstack(all_attention_scores)  # [epochs, 22]
        df = pd.DataFrame(full_array, columns=[f'Node_{i + 1}' for i in range(full_array.shape[1])])
        df.insert(0, 'Epoch', epoch_labels)
        df.to_csv(args.result_path + f'/C3_attention_scores_{str(args.id)}.csv', index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-adj_edge_change_ratio', type=float, default=0.4,
                        help='Ratio of edges to change during perturbation.')  # Edge perturbation
    parser.add_argument('-adj_weight_noise_std', type=float, default=0.2,
    ...
    parser.add_argument('-log_path', type=str, default='', help='The log files path.')
    ...
    parser.add_argument('-spatial_adj_path', type=str, default='', help='Path of saved spatial_adj.')
    parser.add_argument('-time_adj_path', type=str, default='', help='Path of saved time_adj.')
    ...
    parser.add_argument('-tensorboard_path', type=str, default='', help='Path of tensorboard.')
